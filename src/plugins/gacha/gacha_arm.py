...
from .gacha_config import filepath
...
def init_role_arms_list():
    # 初始化卡池数据
    with open(os.path.join(filepath,'config.json'),'r', encoding='UTF-8') as f:
        data = json.load(f)
...
class Gacha(object):
    ...
    def get_png_path(name):
        ...
        role_name_path = os.path.join(filepath, "角色图鉴", str(name) + ".png")
        arms_name_path = os.path.join(filepath, "武器图鉴", str(name) + ".png")
    ...
    def gacha_80(self,frequency=80):
        # 抽一井
        gacha_txt = ""
